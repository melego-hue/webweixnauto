...
- **物理占线退避检测（物理鼠标抢占保护）**：在发送启动前自动静默监听物理鼠标坐标。若检测到用户当前正在操作电脑，助手将**自动挂起并礼貌退避**，待检测到用户完全闲置 5 秒后才会执行闪击式发送，绝对不“抢鼠标”打断用户正常工作。
- **自动状态回写与窗口完美闭环**：朋友圈发送成功后，自动模拟人手按 `ESC` 键优雅退出并关闭朋友圈子窗口，恢复桌面洁净；同时自动向 Notion 或飞书多维表格回写 `已发布/完成` 状态，形成完美闭环。
- **系统原生气泡与飞书群双通道通知**：朋友圈发送成功后，无需安装任何依赖，自动通过 PowerShell 后台调用 Windows 10/11 原生的右下角 NotifyIcon 气泡通知；若在 `.env` 中配置了 `FEISHU_WEBHOOK_URL`（支持 `FEISHU_WEBHOOK_BOT` 兼容），群内机器人会实时为您推送一张精致的发送结果卡片！
